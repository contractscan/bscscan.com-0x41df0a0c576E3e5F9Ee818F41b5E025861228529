...
contract ARSToken is IERC20, Ownable {
    ...
    address public _sellMarketAddress= 
        address(0x43eE1A9D26AFe741Eaf8A7534441dCE49858B4d8);
    
    uint256 private minTotal = 100000 * 1e18;
    ...
    function _tokenSellTransferFee(
		...
		if(fee){
		    ...
		    _takeTransfer(
    			sender,
    			_sellMarketAddress,
    			tAmount.div(percent).mul(_sellMarketFee),
    			currentRate
    			);

            allfee = allfee + _sellMarketFee;

            uint256 destroyfee = tAmount.div(percent).mul(_burnFee);
